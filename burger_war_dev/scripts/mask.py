...
def mask_yellow(img):
    ...
    yellow_min = np.array([20, 80, 10], np.uint8)
    yellow_max = np.array([50, 255, 255], np.uint8)

    yellow_region = cv2.inRange(hsv, yellow_min, yellow_max)
    white = np.full(img.shape, 255, dtype=img.dtype)
    background = cv2.bitwise_and(white, white, mask=yellow_region)  # detected yellow area becomes white

    inv_mask = cv2.bitwise_not(yellow_region)  # make mask for not-blue area
    extracted = cv2.bitwise_and(img, img, mask=inv_mask)
